...
async function syncFromSheets(){
  ...
  try {
    ...
    if(data.Acteurs && data.Acteurs.length>0){
      const newActors=parseActorRows(data.Acteurs);
...
// Mapping positionnel Acteurs : 0=ID, 1=Nom, 2=Rôle, 3=Couleur, 4=Email, 5=Note, 6=Actif ('Non' = archivé, tout le reste = actif — rétro-compatible avec les lignes sans cette colonne)
function parseActorRows(rows){
  return rows.slice(1).filter(row=>row && row[1] && String(row[1]).trim()).map(row=>({
    id:String(row[0]||uid()),
    name:String(row[1]||''),
    role:String(row[2]||'UX'),
    color:String(row[3]||'teal'),
    active:String(row[6]||'').trim().toLowerCase()!=='non'
  }));
}

// Récupère uniquement l'onglet Acteurs depuis Google Sheets, sans toucher au reste de la DB locale (projets, tâches en cours de saisie, etc.)
async function fetchServerActors(){
  try{
    const json = await jsonp(SHEETS_URL, { action:"read", t:Date.now() });
    if(json.ok && json.data.Acteurs && json.data.Acteurs.length>0){
      return parseActorRows(json.data.Acteurs);
    }
  } catch(err){
    console.warn('Impossible de récupérer les acteurs distants avant écriture, on continue avec la liste locale', err);
  }
  return null;
}

// Enrobe toute modification d'acteur (ajout/édition/archivage/suppression) : on récupère d'abord la liste
...
async function mutateActors(mutateFn, savingMsg){
  ...
  if(server){
    ...
    const localOnly=DB.actors.filter(a=>!serverIds.has(a.id)); // pas encore synchro côté serveur
    DB.actors=[...server, ...localOnly];
  }
  mutateFn();
  saveDB();
  render();
