...
// N'affiche pas les acteurs archivés dans les listes d'assignation — sauf si c'est déjà la valeur sélectionnée (on ne veut pas perdre l'affectation existante en rouvrant un formulaire)
function actorOpts(sel=''){return DB.actors.filter(a=>a.active!==false||a.id===sel).map(a=>`<option value="${a.id}"${a.id===sel?' selected':''}>${a.name} (${a.role})${a.active===false?' — archivé':''}</option>`).join('');}
...
async function saveNewActor(){
  ...
  const color=document.getElementById('f-acolor').value;
  closeModal();
  await mutateActors(()=>{
    DB.actors.push({id:uid(),name,role,color,active:true});
  }, '👤 '+name+' ajouté(e) à l\'équipe');
}

function openEditActor(aid){
  const a=DB.actors.find(x=>x.id===aid);if(!a)return;
  openModal(`<div class="modal-title">Modifier ${a.name}</div>
    <div class="field"><label>Nom</label><input type="text" id="f-aname" value="${a.name}"></div>
    <div class="field"><label>Rôle</label><select id="f-arole">${ROLES.map(r=>`<option${r===a.role?' selected':''}>${r}</option>`).join('')}</select></div>
    <div class="field"><label>Couleur</label>${colorPicker(a.color,'acolor')}</div>
    <div class="modal-actions"><button class="btn" onclick="closeModal()">Annuler</button><button class="btn primary" onclick="saveEditActor('${aid}')">Enregistrer</button></div>`);
}
async function saveEditActor(aid){
  const name=document.getElementById('f-aname').value.trim();if(!name)return;
  const role=document.getElementById('f-arole').value;
  const color=document.getElementById('f-acolor').value;
  closeModal();
  await mutateActors(()=>{
    const a=DB.actors.find(x=>x.id===aid);
    if(a){a.name=name;a.role=role;a.color=color;}
  }, '✎ '+name+' modifié(e)');
}

// Archiver = "quitte l'équipe" sans perdre l'historique : n'apparaît plus dans les listes d'assignation,
// mais reste visible sur les tâches/heures déjà enregistrées et sur son profil (accessible via "Voir les anciens").
async function archiveActor(aid){
  const a=DB.actors.find(x=>x.id===aid);if(!a)return;
  if(!confirm(a.name+' quitte l\'équipe ?\n\nSes tâches et heures déjà enregistrées restent visibles, mais il/elle n\'apparaîtra plus dans les listes d\'assignation. Tu pourras le/la réactiver à tout moment.'))return;
  await mutateActors(()=>{
    const target=DB.actors.find(x=>x.id===aid);
    if(target) target.active=false;
  }, a.name+' archivé(e)');
}
async function reactivateActor(aid){
  const a=DB.actors.find(x=>x.id===aid);if(!a)return;
  await mutateActors(()=>{
    const target=DB.actors.find(x=>x.id===aid);
...
async function deleteActor(aid){
  const a=DB.actors.find(x=>x.id===aid);if(!a)return;
  const tasks=DB.projects.flatMap(p=>p.steps.filter(s=>s.assignee===aid));
  const hours=(DB.hours||[]).filter(h=>h.actor===aid);
  let msg='Supprimer définitivement '+a.name+' ?\n\nCette action est irréversible.';
  if(tasks.length||hours.length){
    msg+='\n\n⚠ '+tasks.length+' tâche(s) et '+hours.length+' saisie(s) d\'heures resteront associées à cet acteur, mais son nom ne s\'affichera plus correctement.\nPréfère "Archiver" pour garder l\'historique lisible.';
  }
  if(!confirm(msg))return;
  await mutateActors(()=>{
    DB.actors=DB.actors.filter(x=>x.id!==aid);
  }, a.name+' supprimé(e)');
}
