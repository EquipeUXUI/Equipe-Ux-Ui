...
// Statut d'un projet pour l'affichage couleur du dashboard (rouge/amber/teal) :
// retard avéré > échéance à ≤7 jours (même horizon que les "Alertes & échéances proches") > à jour
...
function computeAlerts(){
  ...
  const lateProjsList=DB.projects.filter(p=>projLate(p));
  ...
  DB.projects.forEach(p=>{
    if(p.actors&&p.actors.length===1&&!(p.steps.length&&p.steps.every(s=>s.done))){
      alerts.push({icon:'👤',color:'amber',title:`${actor(p.actors[0]).name} est la seule personne sur ${p.name}`,detail:'Aucun filet en cas d\'absence'});
    }
  });
  return alerts;
}
...
function meteoStatus(){
  const lateP=DB.projects.filter(p=>projLate(p)).length;
  const prog=DB.projects.length?Math.round(DB.projects.reduce((s,p)=>s+projProg(p),0)/DB.projects.length):0;
...
// ─── STATE ───────────────────────────────────────────────────────────────
let state={view:'global',projId:null,tab:'steps',taskId:null,accordionOpen:{},dashProjFilter:'all',dashPoleFilter:'all'};
