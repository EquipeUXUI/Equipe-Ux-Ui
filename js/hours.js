...
// ─── ÉDITION D'UNE SAISIE (partagée par les 3 vues : projet / acteur / globale) ──
// state.editingHourId pointe la saisie en cours d'édition (null sinon).
// _editHourRerender garde la fonction de ré-affichage à rappeler après
// enregistrement/annulation — elle dépend du contexte d'où on a cliqué ✎.
let _editHourRerender=null;

function startHourEdit(id, rerenderFn){
  state.editingHourId=id;
  _editHourRerender=rerenderFn;
  rerenderFn();
}
function cancelHourEdit(){
  state.editingHourId=null;
  if(_editHourRerender) _editHourRerender();
}
function updateEditTaskOpts(hid){
  const projId=document.getElementById('eh-proj-'+hid).value;
  const sel=document.getElementById('eh-task-'+hid);
  if(!sel) return;
...
function updateHourEntry(id, patch){
  const h=(DB.hours||[]).find(x=>x.id===id);
  if(!h) return;
  Object.assign(h, patch);
  saveDB();
}
function saveHourEdit(id){
  const actorId=document.getElementById('eh-actor-'+id).value;
  const projId=document.getElementById('eh-proj-'+id).value;
  const taskId=document.getElementById('eh-task-'+id).value;
  const dur=parseFloat(document.getElementById('eh-dur-'+id).value)||0;
  const date=document.getElementById('eh-date-'+id).value;
  const note=document.getElementById('eh-note-'+id).value.trim();
  ...
  updateHourEntry(id,{actor:actorId,projId,taskId,taskName:step?step.name:'Général',dur,date,note});
  state.editingHourId=null;
  showToast('✅ Saisie modifiée');
  if(_editHourRerender) _editHourRerender();
}
// Formulaire d'édition inline — remplace la ligne d'affichage normale quand
// state.editingHourId correspond à cette saisie.
function buildHourEditForm(h){
  const proj=DB.projects.find(p=>p.id===h.projId);
  const wrap=mkEl('div','padding:10px 16px;border-bottom:0.5px solid var(--border);background:var(--surface2);');
  const actorOpts=DB.actors.map(a=>`<option value="${a.id}" ${a.id===h.actor?'selected':''}>${a.name}</option>`).join('');
  const projOpts=DB.projects.map(p=>`<option value="${p.id}" ${p.id===h.projId?'selected':''}>${p.name}</option>`).join('');
  const taskOpts=proj?proj.steps.map(s=>`<option value="${s.id}" ${s.id===h.taskId?'selected':''}>${s.name}</option>`).join(''):'';
  wrap.innerHTML=`
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr 70px 120px;gap:6px;margin-bottom:6px;">
      <div><div style="font-size:10px;color:var(--text3);margin-bottom:2px">Équipier</div>
        <select id="eh-actor-${h.id}" style="width:100%;font-size:12px;padding:5px 6px;border-radius:4px;border:0.5px solid var(--border-md);background:var(--bg);color:var(--text);height:30px;">${actorOpts}</select></div>
      <div><div style="font-size:10px;color:var(--text3);margin-bottom:2px">Projet</div>
        <select id="eh-proj-${h.id}" onchange="updateEditTaskOpts('${h.id}')" style="width:100%;font-size:12px;padding:5px 6px;border-radius:4px;border:0.5px solid var(--border-md);background:var(--bg);color:var(--text);height:30px;"><option value="">— Projet —</option>${projOpts}</select></div>
      <div><div style="font-size:10px;color:var(--text3);margin-bottom:2px">Tâche</div>
        <select id="eh-task-${h.id}" style="width:100%;font-size:12px;padding:5px 6px;border-radius:4px;border:0.5px solid var(--border-md);background:var(--bg);color:var(--text);height:30px;"><option value="">— Général —</option>${taskOpts}</select></div>
      <div><div style="font-size:10px;color:var(--text3);margin-bottom:2px">Durée</div>
        <input type="number" id="eh-dur-${h.id}" min="0.25" max="24" step="0.25" value="${h.dur}" style="width:100%;font-size:12px;padding:5px 6px;border-radius:4px;border:0.5px solid var(--border-md);background:var(--bg);color:var(--text);height:30px;"></div>
      <div><div style="font-size:10px;color:var(--text3);margin-bottom:2px">Date</div>
        <input type="date" id="eh-date-${h.id}" value="${h.date}" style="width:100%;font-size:12px;padding:5px 6px;border-radius:4px;border:0.5px solid var(--border-md);background:var(--bg);color:var(--text);height:30px;"></div>
    </div>
    <div style="display:flex;gap:6px;align-items:end;">
      <input type="text" id="eh-note-${h.id}" value="${(h.note||'').replace(/"/g,'&quot;')}" placeholder="Note (optionnel)" style="flex:1;font-size:12px;padding:5px 8px;border-radius:4px;border:0.5px solid var(--border-md);background:var(--bg);color:var(--text);height:30px;">
      <button class="btn primary" title="Enregistrer" onclick="saveHourEdit('${h.id}')">✓ Enregistrer</button>
      <button class="btn" title="Annuler" onclick="cancelHourEdit()">Annuler</button>
    </div>`;
  return wrap;
}
...
function renderProjectHoursTab(proj, container){
  ...
  const renderRow=(h)=>{
    if(state.editingHourId===h.id) return buildHourEditForm(h);
    ...
    row.innerHTML=`
      ...
      <button class="btn-icon sm" title="Modifier" onclick="startHourEdit('${h.id}', function(){renderTabContent(DB.projects.find(p=>p.id==='${proj.id}'))})">✎</button>
...
function renderActorHours(container, aid){
  ...
  const renderRow=(h)=>{
    ...
    row.innerHTML=`
      ...
      <button class="btn-icon sm" title="Modifier" onclick="startHourEdit('${h.id}', function(){renderActorView('${aid}')})">✎</button>
...
function renderHourEntries(container, entries, compact=false){
  ...
  entries.sort((a,b)=>b.date.localeCompare(a.date)).forEach(h=>{
    if(state.editingHourId===h.id){ table.appendChild(buildHourEditForm(h)); return; }
    const proj=DB.projects.find(p=>p.id===h.projId);
    const a=actor(h.actor);const ap=PAL[a.color]||PAL.teal;
    const pp=proj?PAL[proj.color]||PAL.teal:PAL.teal;
    ...
    row.innerHTML=`
      ...
      <button class="btn-icon sm" title="Modifier" onclick="startHourEdit('${h.id}', renderHoursView)">✎</button>
